...
- Field lessons distilled: a ready claim reviewed word-by-word with block power, residual nominal
  label declared; a "it's just environment" dismissal hiding one real marker sentinel; a
  golden-record holding with an embedded "never re-count the engine multiplier" trap held across a
  week of test batteries.
